...
// TODO: we need to modify lines 19-26 since the encoder we're using does not have the holes
...
uint16_t scan_dist_buffer0[CAPT_POINTS_CNT];    // TODO: why does he have two buffers?
...
{
  capture_ctr_init_capture_timer(); // init the timer
...
{
  if (device_state_mask & LASER_DISABLE_MASK)   // if laser is disabled, we do not perform distance measurement
  ...
  if (scan_dist_raw_data_ready_flag && dist_measurenent_enabled)
  {
    ...
    for (uint16_t i = 0; i < CAPT_POINTS_CNT; i++) // going through each of the points? This is different from the # of holes in the encoder
    {
      capture_ctr_dist_write_ptr[i] = 
        dist_measurement_process_data(
          capture_ctr_read_ptr[i].start_value,
          capture_ctr_read_ptr[i].width_value); // TODO: check what is this width value?
    }
    //TEST_GPIO->ODR &= ~TEST_PIN;
    capture_ctr_switch_dist_buffers();
    mavlink_send_scan_data(capture_ctr_dist_read_ptr, CAPT_POINTS_CNT);     // this sends the data to somewhere, the function is defined in another file
...
{
  TEST_GPIO->ODR |= TEST_PIN;   // TODO: what does this do?
  if (TIM_GetITStatus(CAPTURE_TIMER, CAPTURE_TIMER_IT_FLAG) != RESET)   // TODO: this function is also imported from somewhere else
  {
...
// Called from encoder interrupt
void capture_ctr_encoder_zero_event(void)   // this is called at encoder 0 mark
{
...
// TODO: not sure the difference of functions 164 and 180
void capture_ctr_switch_buffers(void)
{
