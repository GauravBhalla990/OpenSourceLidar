...
{ //initing the comparator and encoder timer
...
void ENCODER_COMP_TIMER_IRQ_HANDLER(void)
{
  if (TIM_GetITStatus(ENCODER_COMP_TIMER, ENCODER_COMP_TIMER_IT_FLAG) != RESET)
  {
    ...
    //updating the capture values based on now and based on timer
    ...
    //finding the difference to get the period
...
void encoder_proc_zero_crossing(void)
{
  ...
  if (encoder_proc_event_cnt != ENCODER_HOLES_CNT) //checking encoder holes
  {
  ...
  encoder_proc_rotation_period_us = 
    ...
    (SystemCoreClock / 1000000); //finding rotation period
...
void encoder_proc_init_comparator(void)
{
//mostly just commands from the included h files
  ...
  GPIO_InitStructure.GPIO_Pin = ENCODER_COMP_PIN; //pin
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AN; //setting mode
...
void encoder_proc_init_encoder_timer(void)
{
 //similar instructions from the data sheet
